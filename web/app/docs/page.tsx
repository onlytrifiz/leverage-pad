...
export const metadata = { title: "docs · multiply.cash" };

/**
 * I docs del protocollo: come funziona il motore, cosa garantiscono i
 * contratti, cosa resta responsabilita' dell'operatore. Scritto per chi
 * verifica, non per chi crede sulla parola.
 * Layout docs: sidebar sinistra sticky con scrollspy, contenuto a destra.
 */
...
const GROUPS: DocsGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "subwallets", title: "Sub-wallet treasuries" },
    ],
  },
  {
...
const PARAMS = [
  ...
  { label: "fee split", value: "100% perp" },
  { label: "perp opens at", value: "$20" },
  { label: "top-up step", value: "$20" },
  { label: "leverage", value: "2–20x isolated" },
  { label: "take profit", value: "+20/50/100%" },
  { label: "profit split", value: "75% burn · 25% treasury" },
  { label: "buyback floor", value: "$25" },
  { label: "buyback per tick", value: "≤ $25" },
  { label: "keeper tick", value: "15s" },
];

export default function DocsPage() {
  return (
    <div className="flex gap-10 pt-10 pb-8">
      ...
      <aside className="sticky top-20 hidden h-fit w-[200px] shrink-0 self-start lg:block">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="lbl mt-6 block border-t border-line pt-4 transition-colors hover:text-ink"
        >
          whitepaper.pdf ↗
        </a>
      </aside>
      ...
      <div className="min-w-0 max-w-[720px] flex-1">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <span className="lbl">protocol docs · v1</span>
          <a
            href="/whitepaper.pdf"
            target="_blank"
            rel="noreferrer"
            className="lbl transition-colors hover:text-ink lg:hidden"
          >
            whitepaper.pdf ↗
          </a>
        </div>
        ...
        {/* indice mobile */}
        <div className="panel mt-6 px-5 py-4 lg:hidden">
...
realized profit → withdrawn on-chain
  ...
  └─ 25% protocol treasury (the only protocol revenue)`}</pre>
          ...
          <Section id="glance" title="At a glance">
            <div className="panel grid grid-cols-2 gap-px overflow-hidden bg-line/50 sm:grid-cols-3">
              {PARAMS.map((p) => (
                <div key={p.label} className="bg-panel px-4 py-3">
                  <div className="lbl mb-1">{p.label}</div>
                  <div className="num text-[13px] font-semibold text-ink">{p.value}</div>
                </div>
              ))}
            </div>
            <p>
          ...
          <Section id="subwallets" title="Sub-wallet treasuries">
            <p>
